...
echo "${kubeconfig}" > /mnt/kubeconfig
...
cat > kubeconfig.xml <<EOF
<com.microsoft.jenkins.kubernetes.credentials.KubeconfigCredentials plugin="kubernetes-cd">
  <scope>GLOBAL</scope>
  <id>kube</id>
  <description></description>
  <kubeconfigSource class="com.microsoft.jenkins.kubernetes.credentials.KubeconfigCredentials\$FileOnMasterKubeconfigSource">
    <kubeconfigFile>/mnt/kubeconfig</kubeconfigFile>
  </kubeconfigSource>
</com.microsoft.jenkins.kubernetes.credentials.KubeconfigCredentials>
EOF
...
sudo java -jar jenkins-cli.jar -s http://localhost:8080/ -auth admin:"$(sudo cat /var/lib/jenkins/secrets/initialAdminPassword)" create-credentials-by-xml system::system::jenkins "(global)" < kubeconfig.xml
